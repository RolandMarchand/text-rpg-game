 ...
 *
 * It is safe to cast uninitialized vectors to any other vector type.
 *
 * It is safe to cast initalized vectors to other vectors as long as their
 * element sizes are equal. e.g. VectorVoidPtr -> VectorCharPtr is supported
 * (both 8 bytes), but VectorChar -> VectorCharPtr is undefined behavior (1 byte
 * vs 8 bytes).
 *
